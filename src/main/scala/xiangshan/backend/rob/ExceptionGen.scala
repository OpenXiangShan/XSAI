...
class ExceptionGen(params: BackendParams)(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper {
  ...
  val matrix_wb = io.wb.zip(wbExuParams).filter(
    _._2.fuConfigs.exists(x => FuType.FuTypeOrR(x.fuType, FuType.matrixAll))
  ).map(_._1)

  val writebacks = Seq(csr_wb, load_wb, store_wb, varith_wb, vls_wb, matrix_wb).filter(_.nonEmpty)
