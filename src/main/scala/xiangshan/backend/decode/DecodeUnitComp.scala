...
class DecodeUnitComp()(implicit p : Parameters) extends XSModule with DecodeUnitConstants with VectorConstants {
  ...
  switch(typeOfSplit) {
    ...
    is(UopSplitType.VEC_XX8) {
      for (index <- 0 until MAX_VLMUL) {
        csBundle(index).lsrc(1) := src2 + index.U
        csBundle(index).lsrc(2) := dest + (index / 4).U
        csBundle(index).ldest := dest + (index / 4).U
        csBundle(index).uopIdx := index.U
      }
    }
    is(UopSplitType.VEC_MXFP8) {
      for (i <- 0 until MAX_VLMUL) {
        csBundle(i).lsrc(0) := src1 + i.U
        csBundle(i).lsrc(1) := src2
        csBundle(i).lsrc(2) := dest + (i / 4).U
        csBundle(i).ldest := dest + (i / 4).U
        csBundle(i).uopIdx := i.U
      }
    }
    is(UopSplitType.VEC_MXFP4) {
      for (i <- 0 until MAX_VLMUL) {
        csBundle(i).lsrc(0) := src1 + i.U
        csBundle(i).lsrc(1) := src2
        csBundle(i).lsrc(2) := dest + (i / 8).U
        csBundle(i).ldest := dest + (i / 8).U
        csBundle(i).uopIdx := i.U
      }
    }
  ...
  // when vstart is not zero, the last uop will modify vstart to zero
  // therefore, blockback and flush pipe
  fixedDecodedInst(numOfUop - 1.U).flushPipe := latchedInst.flushPipe
