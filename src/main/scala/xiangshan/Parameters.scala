  ...
  val vfSchdParams = {
    ...
    SchdBlockParams(Seq(
      IssueBlockParams(Seq(
        ...
        ExeUnitParams("VFEX1", Seq(VfaluCfg, VfcvtCfg, Vfexp2Cfg, VipuCfg, VSetRvfWvfCfg), Seq(VfWB(port = 0, 1), V0WB(port = 0, 1), VlWB(port = vfSchdVlWbPort, 0), IntWB(port = 1, 1), FpWB(port = 0, 1)), Seq(Seq(VfRD(0, 1)), Seq(VfRD(1, 1)), Seq(VfRD(2, 1)), Seq(V0RD(0, 1)), Seq(VlRD(0, 1)))),
